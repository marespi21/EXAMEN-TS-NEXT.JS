...
export default function ReservaForm() {
    ...
    const [availableTables, setAvailableTables] = useState<AvailableService[]>([]);
    ...
    // Actualiza un campo del formulario y borra su error
    ...
    // Consulta la API de servicios disponibles para la fecha/hora elegida
    async function handleSearchTables() {
        ...
        try {
            ...
            const res = await fetch(`/api/reservas/servicios?${params}`);
            const json = await res.json();
            if (!res.ok) throw new Error(json.message);
            setAvailableTables(json.data);
            setTablesSearched(true);
        } catch (err) {
            setSubmitError(err instanceof Error ? err.message : "Error al buscar servicios");
        } finally {
            setLoadingTables(false);
        }
    }

    // Envía la reserva a la API (el userId lo toma del JWT en el servidor)
    async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        setSubmitError(null);

        const validationErrors = validateForm(form);
        if (!form.serviceId) validationErrors.serviceId = "Selecciona un servicio";
        ...
        try {
            const res = await fetch("/api/reservas", {
                ...
                // Ya no enviamos userId — la API lo lee del JWT en la cookie
    ...
    function resetForm() {
        ...
        setForm({ name: "", email: "", phone: "", date: "", time: "", partySize: 1, serviceId: "" });
        setAvailableTables([]);
        setTablesSearched(false);
    }
    ...
    if (confirmation) {
        return (
            <div className="max-w-md mx-auto mt-16 p-6 bg-green-50 border border-green-300 rounded-xl text-center">
                ...
                <div className="text-gray-700 space-y-1 text-sm">
                    <p>Fecha: <strong>{confirmation.date}</strong> a las <strong>{confirmation.time}</strong></p>
                    <p>Personas: <strong>{confirmation.partySize}</strong></p>
                    <p>Servicio: <strong>{confirmation.serviceName}</strong></p>
                </div>
                <div className="mt-6 flex gap-3 justify-center">
                    <button onClick={resetForm} className="text-sm text-green-700 underline">
                        Hacer otra reserva
                    </button>
                    <a href="/dashboard" className="text-sm text-blue-700 underline">
                        Ver mis reservas
                    </a>
                </div>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <input
                        type="number" name="partySize" value={form.partySize} onChange={handleChange}
                        min={1} max={20}
                        className="w-full border rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
                    />
            ...
            {/* Botón buscar mesas */}
            <button
                type="button" onClick={handleSearchTables} disabled={loadingTables}
            ...
            >
                {loadingTables ? "Buscando servicios..." : "Ver servicios disponibles"}
            </button>
                ...
                <div>
                    {availableTables.length === 0 ? (
                        <p className="text-sm text-orange-600 bg-orange-50 border border-orange-200 rounded-lg p-3">
                            No hay servicios disponibles para esa fecha, hora y número de personas.
                        </p>
                    ) : (
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {availableTables.map((table) => (
                                    <button
                                        key={table.id} type="button"
                                        onClick={() => setForm((prev) => ({ ...prev, serviceId: table.id }))}
                                        className={`border rounded-lg p-3 text-sm text-left transition ${
                                            form.serviceId === table.id
                                    ...
                                    >
                                        ...
                                        <span className="block text-gray-500 text-xs">
                                            Hasta {table.max_concurrent} personas
                                        </span>
                                    </button>
                                ))}
                            </div>
            ...
            {/* Confirmar */}
            <button
                type="submit" disabled={loadingSubmit || !form.serviceId}
